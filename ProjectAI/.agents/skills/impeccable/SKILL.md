...
0. Ensure the repo-local skill harness exists. If `./.agents/skills/impeccable/SKILL.md` is missing, do not block. Bootstrap it from the installed skill, then continue:

   PowerShell:

   ```powershell
   node "$env:USERPROFILE\.agents\skills\impeccable\scripts\ensure-project-skill.mjs"
   ```

   POSIX shell:

   ```bash
   node "$HOME/.agents/skills/impeccable/scripts/ensure-project-skill.mjs"
   ```

   If those candidates are missing but the session advertises an Impeccable skill path, run `scripts/ensure-project-skill.mjs` from that advertised path directly. The script copies the needed `SKILL.md`, `reference/`, `scripts/`, and `agents/` files into `.agents/skills/impeccable`; it is a no-op when they already exist.
