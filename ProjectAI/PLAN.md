...
- Core mechanic: student writes a prompt → a mocked "agent" pretends to run it (fake thinking + pre-scripted output) → a **real LLM silently judges the prompt** against a rubric → weak prompt = the agent avatar pops up with coaching → student iterates → **Submit** locks in the attempt → aura points by tier.
...
│   │   ├── execute.ts      # runs the student's prompt for real → typed Artifact (+ offline fallback)
...
│   └── components/         # AgentAvatar, AuraBadge, QuestCard, Confetti, etc.
│       └── quest/artifacts/ # SlidesDeck, WebsiteFrame, PlanDoc, ResearchCard, ArtifactBubble
```
...
2. **The executor** (`lib/execute.ts`) — *actually runs the student's prompt* and returns a typed **artifact** the chat renders as the real thing: a slide deck, a website, a plan doc, a research brief. No more canned per-tier text.

```
POST /api/run {questId, prompt} → {verdict: JudgeVerdict, artifact: Artifact, artifactFallback: boolean, trapSprung: boolean}
```
...
**Artifact schemas** (`lib/types.ts`, additive): `SlidesArtifact` · `WebsiteArtifact` · `PlanArtifact` · `ResearchArtifact`, union `Artifact`; each quest declares `artifact: "slides" | "website" | "plan" | "research"`. Every LLM response passes through one validator (`coerce`) that clamps lengths, drops junk and rejects empty output.

**Offline fallback:** `quest.mockOutputs[tier]` now holds a **JSON string of an Artifact** per tier, hand-written to be realistic. On timeout (12s), HTTP error or parse failure the route serves the fallback artifact for the *judge's* tier and sets `artifactFallback: true`, which stamps "offline mode" on the artifact bubble. Same validator, so fallback and live output are guaranteed identical in shape.

**Chat rendering** (`components/quest/artifacts/`): `ArtifactBubble` dispatches by kind → `SlidesDeck` (16:9 viewer, arrows + dots + counter), `WebsiteFrame` (browser chrome + rendered mini landing page), `PlanDoc` (goal callout, numbered steps with tool chips, done-criteria checklist), `ResearchCard` (facts with per-fact source line, citations in small type — where the planted fakes visibly sit).
