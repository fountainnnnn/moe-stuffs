...
const RUBRIC_GUIDE: Record<string, string> = {
  ...
  stack: "Stack decision — the prompt either names the technology and hosting, or explicitly asks the agent to compare a small number of suitable beginner-friendly options, explain tradeoffs and wait for the student's choice",
    ...
    switch (key) {
      ...
      case "stack":
        return has(p, STACK_WORDS)
          || (has(p, ["recommend", "suggest", "compare", "option", "choice"])
            && has(p, ["tool", "platform", "website builder", "no-code", "no code"]));
      case "data":
...
const FALLBACK_ADVICE: Record<string, string> = {
  ...
  stack: "Choose the tools, or ask for two beginner-friendly options with tradeoffs before the agent continues.",
